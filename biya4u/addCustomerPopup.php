	...
	*/
	...
	if($_POST['submit']) {
		...
		*/
		...
		$addCustomerId = mysqli_insert_id($conn);
		//echo "[SQL]: " . $SQLInsert . "<br>";
		
		$statusMsg = "";
		
		if( $successInsertion ) {
			$statusMsg = "Successfully added a customer.";
			echo "<script>\n";
			echo "	parent.loadSelectedCustomerInfos('" . "C" . $addCustomerId . "');\n";
			echo "	parent.closeBPopup();\n";
			echo "</script>";
		}
		else {
