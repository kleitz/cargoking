<?php 
  ...
  if( $stationId != "" ) {
    ...
    echo "  self.location='excess_baggage_list.php?action=delete&success=true';";
    echo "</script>";
  }

  $stationFilter = $stationId == "" ? "" : " AND station_id = " . $stationId;
  $total_results = (mysqli_query($conn, " select count(id) as row_count from vw_loginusers where user_type_id = " . EXCESS_BAGGAGE_ID  . $stationFilter . " limit 1 ")); 
  ...
  $SQLQuery = "SELECT * FROM vw_loginusers WHERE user_type_id = " . EXCESS_BAGGAGE_ID  . $stationFilter . " ORDER BY full_name desc LIMIT " . $offset . ", $limit ";
...
<head>
    ...
    <title>Cargo King: Excess Baggage List</title>
    ...
    <script type="text/javascript">
      ...
      $(document).ready(function(){
        ...
        $("#tblExcessBaggageList tr:odd").addClass("oddRow");
        $("#tblExcessBaggageList tr:even").addClass("evenRow");

        if( action == "add" && success == "true"){
          $("#divStatus").show();
          $("#divStatusMessage").html("Successfully added station excess baggage.").addClass("success");
        }
        else if( action == "add" && success == "false"){
          $("#divStatus").show();
          $("#divStatusMessage").html("Unable to add station excess baggage.").addClass("error");
        }
        else if( action == "update" && success == "true"){
          $("#divStatus").show();
          $("#divStatusMessage").html("Successfully updated station excess baggage.").addClass("success");
        }
        else if( action == "delete" && success == "true"){
          $("#divStatus").show();
          $("#divStatusMessage").html("Successfully removed station excess baggage.").addClass("success");
        }
        else if( action == "delete" && success == "false"){
          $("#divStatus").show();
          $("#divStatusMessage").html("Cannot deleted active station excess baggage.").addClass("error");
        }
        else {
        ...
        function deleteStationExcessBaggage(deleteExcessBaggageId) {
          ...
          if( confirmDelete = confirm("Are you sure you want to remove station excess baggage?") ){
            location.href = "excess_baggage_list.php?del_id=" + deleteExcessBaggageId;
          }
  ...
  <center>
    ...
    <div class="containers contents">
      <div style="width: 800px; padding: 10px;">
        ...
        <!-- Title -->      
        <div align="center" class="title">Station Excess Baggage staffs</div>
        ...
        <div class="clear" align="center">
          <div>
            <tr>
                <td>
                  <table id="tblExcessBaggageList" width="100%" border="0"  cellspacing="0" cellpadding="5" align="center" style="border:5px solid #bbbbbb; margin-bottom: 10px;">
                    <tr>
                    ...
                    <tr>
                      ...
                      <td width="13%" style="padding-left: 15px;">
                        ...
                        <a class="actionButtons" href="javascript:void(0);" onclick="deleteStationExcessBaggage(<?php echo  $fet_2['id']; ?>)"><img src="images/flat_icons/delete30.png" class="imageButtonFlat" alt="X" /></a>
                      </td>
                    </tr> 
                    <?php 
                        }
          ...
          <!-- START: Add Button -->
          <p align="right">
            <a id="lnkButton" href="javascript:void(0)" onclick="location.href='add_excess_baggage.php'" class="flatButton">Add Station Excess Baggage</a>
          </p>
          <!-- END: Add Button -->
          <br />
