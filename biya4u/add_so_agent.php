<?php 
  ...
  if($_POST['submit']) {
    ...
    $SQLInsert .= " values (" . SO_AGENT_ID . ", '" . $code . "', '" . $identificationType . "', '" . $identificationNumber . "', '" . $firstName . "', '" . $lastName . "', '" . $middleName . "', '" . $area_name . "', '" . $phoneNumber . "', '" . $emailAddress . "', '" . $username . "', '" . $password . "', '" . $login_id . "', now(), '" . $login_id . "', now())";
    $successInsertion = mysqli_query($conn,  $SQLInsert );

    if( $successInsertion ) {
      echo "<script type=\"text/javascript\">";
      echo "  self.location='so_agent_list.php?action=add&success=true';";
      echo "</script>";
    }
    else {
...
<head>
    ...
    <title>Cargo King: Add Satellite Office Agent</title>
    ...
    <style>
      ...
      #sel_area_name, #selIdentificationType, #sel_area {
        height: 32px;
        width: 323px;
      }
    </style>
      ...
      $(document).ready(function(){
        ...
        $("#frmStationAdmin").validate({
          ...
          messages: {
            ...
            area: {
              required: "Please select delivery area or branch."
            },
            firstName: {
        ...
        $("#sel_area_name").bind("change", function(){
          var area_name = $(this).val();
          var strOptions = "<option value=\"\">--[Select]--</option>";
          $.getJSON("populate_Branches.php",{ area_name: area_name}, function(data){
            $("#sel_area").empty();
            $.each(data, function(idx, obj){
              console.log("[IDX]: " + idx + "\t[LABEL]: " + obj.label + "\t[VALUE]: " + obj.value);
              strOptions += "<option value=\"" + obj.value + "\">" + obj.label + "</option>\n";
            });
            $("#sel_area").append(strOptions);
          });
        });
      });
  ...
  <center>
    ...
    <div class="containers contents">
      <div class="contentContainer">
        ...
        <!-- Title -->      
        <div align="center" class="title">Add Satellite Office Agent</div>
          ...
          <div>
            ...
            <form id="frmStationAdmin" name="frmStationAdmin" action="" method="post">           
                <div class="profileContainer">
                  ...
                  <p>
                    <span class="required">*</span>
                    <label for="sel_area" class="fieldLabel">Satellite Office:</label>
                    <select id="sel_area" class="profileField" name="area">
                ...
                <p align="right" style="margin-right: 10px;">
                  <input type="submit" id="btnAddUser" name="submit" value="Add Satellite Office Agent" class="button" />
                </p>
            </form>
          </div>
        </div>
