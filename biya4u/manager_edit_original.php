...
$upd_query = mysqli_query($conn,  "update manager set name='$nam',city='$city',phone='$phone',email='$tin',username='$uname' where id ='$md_edit' ") or die (mysqli_error() );

print "<script>";
print "self.location='manager_confirm.php'";
print "</script>";
...
?>
	<select name="<?=$name?>" style="width:250px;">	  
		...
		?>
		<option value="<?php echo $row['id']; ?>" <?php if($it==$row['id']) { ?> selected="selected"<?php } ?>><?php echo $row['category']; ?></option>
		<?php
			}
		?>
	</select>
	...
	<div align="center" style="background:#FFF;">
		<table width="580" border="0" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF" >
			<tr>
				<td>
					<p>&nbsp;</p>
					<div align="center" class="style2"><strong> Edit Manager Details</strong></div>
				</td>
			</tr>
		  <tr>
			<td >
				...
				<?php 
					$row = array();
					$rsManager = mysqli_query($conn, "select * from manager where id = '$md_edit' ");
					if($rsManager) {
						$row = mysqli_fetch_array($rsManager);	
					}
				?>
